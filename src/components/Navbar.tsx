...
function Navbar({ className }: { className?: string }) {
  ...
  return (
    <div
      className={cn(
        "absolute md:fixed mt-5 Navbar  inset-x-0 max-w-6xl mx-auto z-50",
        className
      )}
    >
      <Menu setActive={setActive}>
        ...
        {user ? (
          ...
          <div className="mainnavbtn absolute right-3 top-1 z-40  ">
            <Link href={"/sign-in"}>
              <button className="Navbarbutton">
                ...
                <span className="text_button z-40  	Bebas">Account</span>
              </button>
            </Link>
          </div>
        )}
